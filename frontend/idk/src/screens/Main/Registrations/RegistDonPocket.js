...
const Pocket = function ({ getAccountId, dataType, myDataItemId, navigation ,route}) {
      ...
      deleteAutoTransferAxios(
        ...
        (err) => {
          if (err.response.data.code === "AT405") {
            ...
            Alert.alert(
              "해당 요청 사용자와 해당 정보의 사용자가 일치하지 않습니다.",
              "",
              [
                {
                  text: "확인",
  ...
  useEffect(() => {
    ...
    if (route?.params?.data?.isChecked && route?.params?.data?.autoTransferId=== myDataItemId.autoTransferId) {
      handleDeletePocket();
    }
  }, [route?.params]);
        ...
        <View className="flex-row flex items-center p-3">
          ...
          <View style={{ flex: 4 }}>
            <Text className="text-lg font-bold">
              {myDataItemId.toAccountBank} {myDataItemId.toAccount}
            </Text>
            {dataType === "자동결제" ? (
              <Text>매월 {myDataItemId.date}일에 청구 대금 납입</Text>
            ) : (
              <Text>
                매월 {myDataItemId.date}일에{" "}
                {formattedNumber(myDataItemId.amount)}원 납입
              </Text>
            )}
          </View>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  navigation.navigate("AuthStack", {
                    screen: "AuthPW",
                    params: {
                      data: { isChecked: false,autoTransferId: myDataItemId.autoTransferId },
                      destination: {
                        stack: "MainStack",
                        screen: "RegistDonPocket",
                      },
                    },
                  });
                }}
